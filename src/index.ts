...
import { LockupDynamic } from "./engine/LockupDynamic";
...
const PRIORITY_GAS_PRICE = GWEI.mul(40);
...
const FLASHBOTS_RELAY_SIGNING_KEY =
  process.env.FLASHBOTS_RELAY_SIGNING_KEY || Wallet.createRandom().privateKey;
...
async function main() {
  ...
  // ======= UNCOMMENT FOR LOCKUP DYNAMIC ==========
  const STREAM_IDS = [174]; // Replace with the actual stream IDs you want to renounce
  const engine: Base = new LockupDynamic(
    provider,
  ...
  > = [
    ...
    ...sponsoredTransactions.map(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (transaction: any, txNumber: string | number) => {
        return {
          transaction: {
            ...transaction,
            gasPrice: gasPrice,
            gasLimit: gasEstimates[txNumber as number],
          },
